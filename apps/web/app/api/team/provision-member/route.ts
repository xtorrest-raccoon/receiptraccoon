 ...
 * relay to that person however they like; no email is ever sent.
 *
...
export async function POST(request: NextRequest) {
  ...
  if (createErr || !created.user) {
    // Most common case: this email already has an account somewhere — the
    // regular Invite flow (which works for existing accounts) is the right
    // path for that, not this one.
    return NextResponse.json(
      { error: createErr?.message ?? "Could not create that account — if the email is already registered, use Invite instead." },
      { status: 409 },
    );
  }
